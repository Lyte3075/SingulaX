...
class SingulaxRuntime{
 constructor(host={}){this.host=host;this.env=Object.create(null);this.running=false;this.keys=new Set();this.buttons=new Set();this.mouse={x:0,y:0};this.touch={x:0,y:0,active:false};this.gamepads={};this.clicked=new Set();this.frame=[];this.assets=Object.create(null);this.scopeStack=[];this.output=[];this.clock=0;
  ...
  E.joystick_x=()=>this.gamepads[0]?.axes?.[0]||0;E.joystick_y=()=>this.gamepads[0]?.axes?.[1]||0;E.joystick2_x=()=>this.gamepads[0]?.axes?.[2]||0;E.joystick2_y=()=>this.gamepads[0]?.axes?.[3]||0;E.joystick_left=()=>E.joystick_x()<-0.25;E.joystick_right=()=>E.joystick_x()>0.25;E.joystick_up=()=>E.joystick_y()<-0.25;E.joystick_down=()=>E.joystick_y()>0.25;E.joystick2_left=()=>E.joystick2_x()<-0.25;E.joystick2_right=()=>E.joystick2_x()>0.25;E.joystick2_up=()=>E.joystick2_y()<-0.25;E.joystick2_down=()=>E.joystick2_y()>0.25;
  const buttonNames={a:0,b:1,x:2,y:3,lb:4,rb:5,lt:6,rt:7};E.button_down=n=>!!this.gamepads[0]?.buttons?.[buttonNames[String(n).toLowerCase()]];E.trigger_value=n=>E.button_down(n)?1:0;
  E.controls={show:(...n)=>this.host.controls?.('show',...n.map(String)),hide:(...n)=>this.host.controls?.('hide',...n.map(String)),only:(...n)=>this.host.controls?.('only',...n.map(String)),reset:()=>this.host.controls?.('reset'),all:()=>this.host.controls?.('all')};
