...
function setupMobileControls(){
  ...
  style.textContent=`
    ...
    .sglx-buttons{right:18px;bottom:24px;display:grid;grid-template-columns:56px 56px;gap:9px}
    .sglx-btn{width:56px;height:56px;border-radius:50%;border:2px solid rgba(255,255,255,.75);background:rgba(45,55,85,.88);color:white;font-weight:800;font-size:18px;box-shadow:0 5px 16px rgba(0,0,0,.35)}
    .sglx-btn:active,.sglx-btn.pressed{transform:scale(.94);background:rgba(80,130,210,.95)}
    #sglxAB{display:grid}
    #sglxXY{display:grid;right:146px;bottom:24px}
    .sglx-dpad{left:18px;bottom:28px;width:144px;height:144px}
    ...
    .sglx-bumpers{left:50%;top:14px;transform:translateX(-50%);display:flex;gap:80px}
    .sglx-bumper{padding:9px 18px;border-radius:14px;border:1px solid rgba(255,255,255,.5);background:rgba(45,55,85,.88);color:white;font-weight:800}
    .sglx-triggers{left:50%;top:58px;transform:translateX(-50%);display:flex;gap:80px}
    .sglx-trigger{padding:7px 16px;border-radius:12px;border:1px solid rgba(255,255,255,.45);background:rgba(35,45,70,.85);color:white;font-weight:800}
  ...
  function addButton(parent,n,label){const b=document.createElement('button');b.className='sglx-btn';b.textContent=label;b.dataset.control=n;const down=e=>{e.preventDefault();b.classList.add('pressed');setButton(n,true)};const up=e=>{e.preventDefault();b.classList.remove('pressed');setButton(n,false)};b.onpointerdown=down;b.onpointerup=up;b.onpointercancel=up;b.onpointerleave=e=>{if(b.classList.contains('pressed'))up(e)};parent.appendChild(b)}
  addButton(ab,'a','A');addButton(ab,'b','B');addButton(xy,'x','X');addButton(xy,'y','Y');root.append(ab,xy);

  const dp=document.createElement('div');dp.id='sglxDpad';dp.className='sglx-control sglx-dpad';[['up','▲'],['left','◀'],['down','▼'],['right','▶']].forEach(([n,l])=>{const b=document.createElement('button');b.className=n;b.textContent=l;const key={up:'ArrowUp',down:'ArrowDown',left:'ArrowLeft',right:'ArrowRight'}[n];const down=e=>{e.preventDefault();keys.add(key);runtime?.setInput({keys:[key],pressed:[key]})};const up=e=>{e.preventDefault();keys.delete(key);runtime?.setInput({up:[key]})};b.onpointerdown=down;b.onpointerup=up;b.onpointercancel=up;dp.appendChild(b)});root.appendChild(dp);

  const bump=document.createElement('div');bump.id='sglxBumpers';bump.className='sglx-control sglx-bumpers';[['lb','LB'],['rb','RB']].forEach(([n,l])=>{const b=document.createElement('button');b.className='sglx-bumper';b.textContent=l;b.onpointerdown=e=>{e.preventDefault();setButton(n,true)};b.onpointerup=e=>{e.preventDefault();setButton(n,false)};b.onpointercancel=b.onpointerup;bump.appendChild(b)});root.appendChild(bump);
  const trig=document.createElement('div');trig.id='sglxTriggers';trig.className='sglx-control sglx-triggers';[['lt','LT'],['rt','RT']].forEach(([n,l])=>{const b=document.createElement('button');b.className='sglx-trigger';b.textContent=l;b.onpointerdown=e=>{e.preventDefault();setButton(n,true)};b.onpointerup=e=>{e.preventDefault();setButton(n,false)};b.onpointercancel=b.onpointerup;trig.appendChild(b)});root.appendChild(trig);
