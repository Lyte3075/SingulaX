...
// Active browser audio and run state.
// Stop() uses these to hard-cancel work from the current run.
...
async function run() {
  ...
  // Starting a new run is a hard restart. Anything from the previous
  // run is cancelled before a new runtime is created.
  stop();
    ...
    new SingulaxRuntime({
      ...
      input: async p => {
        ...
        return await new Promise(resolve => {
          ...
          window._inputGeneration = myGeneration;
          $('stdin').focus();
        });
      },
  ...
  runRuntime.env.joystick_x = () =>
    runRuntime?.gamepads?.[0]?.axes?.[0] || 0;

  runRuntime.env.joystick_y = () =>
    runRuntime?.gamepads?.[0]?.axes?.[1] || 0;

  runRuntime.env.joystick_left = () =>
    runRuntime.env.joystick_x() < -0.25;

  runRuntime.env.joystick_right = () =>
    runRuntime.env.joystick_x() > 0.25;

  runRuntime.env.joystick_up = () =>
    runRuntime.env.joystick_y() < -0.25;

  runRuntime.env.joystick_down = () =>
    runRuntime.env.joystick_y() > 0.25;

  paintFrame();

  try {
    await runRuntime.runProject(project);

    // An older run must never clean up a newer run.
    if (runtime !== runRuntime || myGeneration !== runGeneration) {
      return;
    }

    paintCanvas(latestFrame);

    if (runRuntime.running) {
      log('[finished]');
    }
    ...
    // A stopped run is expected to unwind quietly. Also ignore errors
    // from an older run after a new run has already started.
    if (runtime !== runRuntime || myGeneration !== runGeneration) {
      return;
    }
    ...
    if (window._inputGeneration === myGeneration) {
      window._inputResolve = null;
      window._inputGeneration = null;
    }
  }
}

function stopAudio(name = null) {
  if (name) {
    const target = String(name);
    for (const audio of [...activeAudios]) {
      if (audio.__singulaxName === target) {
        try {
          audio.pause();
          audio.currentTime = 0;
        } catch {}
...
function stop() {
  const hadRuntime = !!runtime;
  const hadAudio = activeAudios.size > 0;
  const hadInput = !!window._inputResolve;

  // Invalidate the current run first so loops/waits stop as soon as
  // their current async operation yields back to the runtime.
  runGeneration++;
  ...
  // Unblock input() if a program is waiting for stdin.
  if (window._inputResolve) {
    const resolve = window._inputResolve;
    window._inputResolve = null;
    window._inputGeneration = null;
    try {
      resolve('');
  ...
  // Kill every audio object started by SingulaX.
  stopAllAudio();

  // Cancel the preview render loop.
  if (paintHandle) {
    cancelAnimationFrame(paintHandle);
  }
  ...
  if (hadRuntime || hadAudio || hadInput) {
    log('[stopped]');
  }
}
...
function playAudio(name, loop = false) {
  ...
  audio.__singulaxName = String(name);
  audio.loop = !!loop;
...
(function setupMobileJoystick(){
  ...
  style.textContent = `
    #mobileJoystick {
      ...
      height: 120px;
      border-radius: 50%;
      background: rgba(35,48,84,.82);
      border: 2px solid rgba(150,180,255,.65);
      box-shadow: 0 8px 28px rgba(0,0,0,.4), inset 0 0 20px rgba(120,150,255,.12);
      z-index: 100;
      touch-action: none;
      user-select: none;
    ...
    #joystickKnob {
      ...
      background: rgba(100,210,255,.9);
      border: 2px solid rgba(255,255,255,.85);
      box-shadow: 0 5px 18px rgba(0,0,0,.4), inset 0 0 16px rgba(255,255,255,.2);
      pointer-events: none;
    }
  ...
  const preview =
    document.getElementById('window-preview');
  ...
  preview.appendChild(joystick);

  let active = false;
  let pointerId = null;

  window.singulaxJoystick = {
    x: 0,
    y: 0,
    active: false
  };
  ...
  function update(x, y) {
    ...
    const max =
      Math.max(1, c.radius - knob.offsetWidth / 2 - 6);
    ...
    const joystickX = dx / max;
    const joystickY = dy / max;

    knob.style.transform =
      `translate(calc(-50% + ${dx}px), calc(-50% + ${dy}px))`;

    window.singulaxJoystick.x = joystickX;
    window.singulaxJoystick.y = joystickY;
    window.singulaxJoystick.active = true;
  ...
  function reset() {
    ...
    window.singulaxJoystick.x = 0;
    window.singulaxJoystick.y = 0;
    window.singulaxJoystick.active = false;
