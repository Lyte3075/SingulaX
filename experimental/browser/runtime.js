...
class SingulaxRuntime{
 constructor(host={}){this.host=host;this.env=Object.create(null);this.running=false;this.keys=new Set();this.buttons=new Set();this.mouse={x:0,y:0};this.touch={x:0,y:0,active:false};this.gamepads={};this.clicked=new Set();this.frame=[];this.assets=Object.create(null);this.scopeStack=[];this.output=[];this.clock=0;
  ...
  const vecArr=v=>[+(v?.x||0),+(v?.y||0),+(v?.z||0)];
  const serialObject=(o)=>({name:o.name,type:o.type,position:vecArr(worldPos(o)),rotation:vecArr(o.rotation),scale:vecArr(o.scale),size:o.size,visible:o.visible,material:{...o.material,color:o.material.color},mesh:o.mesh?{name:o.mesh.name,vertices:o.mesh.vertices.map(v=>[v.x,v.y,v.z]),faces:o.mesh.faces.map(f=>f.slice())}:null});
  const serializeScene=sc=>({name:sc.name,ambient:sc.ambient,background:sc.background,fog:sc.fog,camera:(sc.activeCamera||sc.cameras[0])?(()=>{const c=sc.activeCamera||sc.cameras[0];return {position:vecArr(worldPos(c)),rotation:vecArr(c.rotation),fov:c.fovValue,near:c.near,far:c.far,sensitivity:c.sensitivity}})():null,lights:sc.lights.filter(l=>l.visible).map(l=>({position:vecArr(worldPos(l)),rotation:vecArr(l.rotation),type:l.lightType,intensity:l.intensity,range:l.range,color:l.color})),objects:sc.objects.filter(o=>o.visible&&o.type!=='camera'&&o.type!=='light').map(serialObject)});
